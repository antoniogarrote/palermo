...
(deftest test-publish-consume
  (testing "Should be possible to publish and consume correct messages"
    ...
    (let [consumed (atom [])
      ...
      (publish-job-messages ch test-exchange-1 test-queue message)
      (Thread/sleep 3000)
      (consume-job-messages ch test-exchange-1 test-queue
                            (fn [message]
                              (swap! consumed conj (:content message))))
      ;; first consume, then publish
      (consume-job-messages ch test-exchange-2 test-queue
                            (fn [message]
                              (swap! consumed conj (:content message))))
      (Thread/sleep 3000)      
      (publish-job-messages ch test-exchange-2 test-queue message)
      (Thread/sleep 3000)      
